-- =============================================================================
-- V27: Allow department policy without script.
-- =============================================================================
...
BEGIN
    ...
    IF default_script IS NOT NULL THEN
        ...
        EXECUTE format(
            'INSERT INTO %I.department_call_policies (department_id, call_direction, script_id, prompt_template_id)
             VALUES (NULL, ''external_incoming'', $1, ''eval_external_incoming'')
             ON CONFLICT DO NOTHING',
            schema_name
        ) USING default_script;
        ...
        EXECUTE format(
            'INSERT INTO %I.department_call_policies (department_id, call_direction, script_id, prompt_template_id)
             VALUES (NULL, ''external_outgoing'', $1, ''eval_external_outgoing'')
             ON CONFLICT DO NOTHING',
            schema_name
        ) USING default_script;
    END IF;
