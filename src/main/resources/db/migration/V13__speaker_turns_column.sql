-- =============================================================================
-- V13: Add speaker_turns JSONB column to transcriptions table
--      for storing diarized dialogue (speaker-labeled turns).
-- =============================================================================
...
BEGIN
    FOR tenant IN SELECT db_schema FROM public.tenants LOOP
        EXECUTE format(
            'ALTER TABLE %I.transcriptions ADD COLUMN IF NOT EXISTS speaker_turns JSONB',
            tenant.db_schema
        );
    END LOOP;
END $$;

-- 2. Update create_tenant_schema function to include the new column
--    (replace the transcriptions CREATE TABLE block)
