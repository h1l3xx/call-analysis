-- =============================================================================
-- V8: Update create_tenant_schema() to create bigint timestamps (Unix ms)
-- =============================================================================
--
-- V6 migrated existing tenant schemas from timestamptz -> bigint, but new tenants
-- created after V6 would still get timestamptz columns from V2 function, causing
-- runtime type mismatches with Exposed (which uses Long/bigint columns).

CREATE OR REPLACE FUNCTION public.create_tenant_schema(schema_name TEXT)
RETURNS VOID AS $$
BEGIN
    -- Protect against SQL injection in schema name
